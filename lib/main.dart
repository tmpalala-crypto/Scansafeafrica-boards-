      ...
      drawer: Drawer(
        child: ListView(children: [
            DrawerHeader(decoration: BoxDecoration(color: Colors.green[800]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Icon(Icons.verified_user, color: Colors.white, size: 50), SizedBox(height: 10),
            ...
            Divider(),
            ListTile(leading: Icon(Icons.flag, color: Colors.green[800]), title: Text("CEO Mission - Clean SA"), onTap: (){ Navigator.push(context, MaterialPageRoute(builder: (c)=>CEOMissionPage())); }),
            ListTile(leading: Icon(Icons.shield, color: Colors.orange[800]), title: Text("For Brands - Get Protected"), onTap: (){ Navigator.push(context, MaterialPageRoute(builder: (c)=>BrandProtectionPage())); }),
        ]),
      ),
      body: _pages[_index],
...
class _PplScanPageState extends State<PplScanPage> {
  ...
  Future<void> saveToFirebase() async {
    ...
    try{
      await FirebaseFirestore.instance.collection('scans').add({
        'brand': brandCtrl.text, 'shop': shopCtrl.text, 'lat': lat, 'lng': lng, 'timestamp': FieldValue.serverTimestamp(), 'status': 'reported',
      });
  ...
  @override Widget build(BuildContext context){
    return Scaffold(appBar: AppBar(title: Text("ScanSafeAfrica - PPL SCAN")), body: Padding(padding: EdgeInsets.all(16), child: Column(children: [
      ...
      SizedBox(height: 10),
      ElevatedButton(style: ElevatedButton.styleFrom(minimumSize: Size(double.infinity, 50), backgroundColor: Colors.green[800]), onPressed: saveToFirebase, child: Text("SCAN & REPORT FAKE", style: TextStyle(color: Colors.white))),
    ])));
  }
}
...
// --- BRAND PROTECTION PAGE WITH QR GENERATOR ---
...
class _BrandProtectionPageState extends State<BrandProtectionPage> {
  ...
  void generateQR(){
    ...
    String data = "SSA-REAL|${brandCtrl.text}|${productCtrl.text}|${barcodeCtrl.text}|$secret|Bethelsdorp";
  ...
  @override Widget build(BuildContext context){
    return Scaffold(appBar: AppBar(title: Text("Brand Protection"), backgroundColor: Colors.orange[800]), body: SingleChildScrollView(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 10),
      Text("Scared of duplicate & lawsuit? Register & get Protected QR that cannot be copied - we track if same QR scanned in 2 places."),
      SizedBox(height: 20),
      ...
      if(generatedQR.isNotEmpty) Center(child: Column(children: [
        Container(padding: EdgeInsets.all(15), color: Colors.white, child: QrImageView(data: generatedQR, version: QrVersions.auto, size: 220)),
        SizedBox(height: 10),
        Text(generatedQR, textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: Colors.grey)),
        SizedBox(height: 10),
        Container(padding: EdgeInsets.all(12), color: Colors.green[50], child: Text("✅ SAVED TO FIREBASE\nThis QR is now protected. If someone copies it, your PPL SCAN will alert: SCANNED BEFORE IN ANOTHER LOCATION = FAKE!", style: TextStyle(fontWeight: FontWeight.bold))),
      ])),
    ])));
  }
}
